...
const SESSION_SECRET =
  process.env.SESSION_SECRET || "local-dev-only-secret";
...
function publicCreator(c) {
  return {
    ...
    currency: c.currency,
    messageTiers: c.message_tiers || [],
    sounds: c.sounds || [],
    memes: c.memes || [],
    platformFeePercent: Number(c.platform_fee_percent || 10)
  };
}
...
async function initDB() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS creators (
      ...
      payout JSONB NOT NULL DEFAULT '{"upiId":"","status":"not_configured"}'::jsonb,
      message_tiers JSONB NOT NULL DEFAULT '[]'::jsonb,
...
async function requireCreator(req, res, next) {
  try {
    ...
    const c = await creatorById(u.creator_id);

    if (!c) {
      return res
        .status(403)
        .json({ error: "Creator account missing" });
    }
...
const storage = multer.diskStorage({
  ...
  filename: (_, file, cb) =>
    cb(
      null,
      Date.now() + "-" + cleanName(file.originalname)
    )
});
...
app.get("/creator/:slug", async (req, res) => {
  try {
    const c = await creatorBySlug(req.params.slug);
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO users
       (id,email,password_hash,creator_id)
       VALUES ($1,$2,$3,$4)`,
      [uid, email, passwordHash, cid]
    );
...
app.get("/api/creator/:slug", async (req, res) => {
  try {
    const c = await creatorBySlug(
      req.params.slug
    );
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const platformFee = paid.reduce(
        (a, b) =>
          a + Number(b.platform_fee || 0),
        0
      );

      const creatorNet = paid.reduce(
        (a, b) =>
          a + Number(b.creator_net || 0),
        0
      );
      ...
      const transactions = rows.map(x => ({
        ...
        platformFee: Number(x.platform_fee),
        creatorNet: Number(x.creator_net),
        status: x.status,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const feePct = Number(
        c.platform_fee_percent || 10
      );

      const platformFee =
        Math.round(gross * feePct) / 100;

      const creatorNet =
        Math.round(
          (gross - platformFee) * 100
        ) / 100;
      ...
      const sound = sounds.find(
        x =>
          x.id === String(body.soundId || "") &&
          x.enabled
      );

      const meme = memes.find(
        x =>
          x.id === String(body.memeId || "") &&
          x.enabled
      );
      ...
      res
        .status(500)
        .json({ error: "Payment test failed" });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      await pool.query(
        "UPDATE creators SET payout=$1 WHERE id=$2",
        [JSON.stringify(payout), req.creator.id]
      );
    ...
    try {
      ...
      await pool.query(
        `UPDATE creators
         SET ${key}=$1
         WHERE id=$2`,
        [JSON.stringify(list), req.creator.id]
      );
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Use MP3/WAV/M4A" });
      }

      if (
        !isSound &&
        ![".mp4", ".gif", ".webm"].includes(ext)
      ) {
        return res
          .status(400)
          .json({ error: "Use MP4/GIF/WEBM" });
      }
